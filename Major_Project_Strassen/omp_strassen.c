...
void print(int n, ull** mat){
    for (int i = 0; i < n; i++){
        for (int j = 0; j < n; j++) printf("%16d ", mat[i][j]); 
    }
    printf("\n");
}
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    printf("Matrix Size = %d, Threads = %d, error = %d, time_strassen (sec) = %8.5f, time_naive = %8.5f\n\n", 
	    n, THREAD, error, total_time, total_time_naive);
