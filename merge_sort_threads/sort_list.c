...
void print_list(int *list, int list_size){
    for (int i = 0; i < list_size; i++) printf("[%d] \t %16d\n", i, list[i]);
    printf("--------------------------------------------------------------------\n"); 
}
...
void *sort_list(void* thread){
    ...
    for (level = 0; level < q; level++){
        ...
        /*
        while(my_id <= num_threads){
            int s = 2;
            index = my_id / s;
            if (my_id % s == 0){
                pthread_mutex_lock(&(b[index].count_lock));
                b[index].count ++;
                while(b[index].count < s){
                    pthread_cond_wait(&(b[index].sync), &(b[index].count_lock));
                ...
                if (b[index].count == s){
                    ...
                    while(pthread_cond_wait(&(b[index].release), &(b[index].count_lock)) != 0){
                        pthread_mutex_unlock(&(b[index].count_lock));
                        break;
                    }
                } 
        ...
        if(my_id == num_threads){
            ...
            s /= 2;
            for (; s > 1; s /= 2){
                index = my_id/s - num_threads/s;
                pthread_mutex_lock(&(b[index].count_lock));
                b[index].count = 0;
                pthread_cond_signal(&(b[index].release));
                pthread_mutex_unlock(&(b[index].count_lock));
            }
        }*/
...
int main(int argc, char *argv[])
{
    ...
    q_ = q;     // pass thread level globally
...
// VS: ... parallel merge sort
    ...
    for (int i = 0; i < num_threads; i++){
        thread_id[i] = i;
        status = pthread_create(&threads[i], &attr, sort_list, &thread_id[i]);
